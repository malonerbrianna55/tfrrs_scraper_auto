# TFRRS scraper for Southland conference track & field
...
# Southland conference league page
SOUTHLAND_LEAGUE_URL = "https://www.tfrrs.org/leagues/47.html"
...
def main():
    # Run the Southland scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for Southland Conference.")
    ...
    print(f"Fetching conference page: {SOUTHLAND_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, SOUTHLAND_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="southland_tfrrs_performances.csv",
    )
