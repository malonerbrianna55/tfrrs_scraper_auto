# TFRRS scraper for Summit conference track & field
...
# SEC conference league page
SUMMIT_LEAGUE_URL = "https://www.tfrrs.org/leagues/98.html"
CONFERENCE_NAME = "SUMMIT"
...
def main():
    # Run the SUMMIT scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for SUMMIT Conference.")
    ...
    print(f"Fetching conference page: {SUMMIT_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, SUMMIT_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="summit_tfrrs_performances.csv",
    )
