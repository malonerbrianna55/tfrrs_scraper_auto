# TFRRS scraper for Ivy League conference track & field
...
IVY_LEAGUE_URL = "https://www.tfrrs.org/leagues/55.html"
...
def main():
    # Run the BIvy League scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for Ivy League Conference.")
    ...
    print(f"Fetching conference page: {IVY_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, IVY_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="ivy_league_tfrrs_performances.csv",
    )
