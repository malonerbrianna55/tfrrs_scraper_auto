# TFRRS scraper for Mid American conference track & field
...
MID_AMERICAN_LEAGUE_URL = "https://www.tfrrs.org/leagues/70.html"
...
def main():
    # Run the Mid American scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for Mid-American Conference.")
    ...
    print(f"Fetching conference page: {MID_AMERICAN_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, MID_AMERICAN_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="mid_american_tfrrs_performances.csv",
    )
