# TFRRS scraper for Sun Belt conference track & field
...
SUN_BELT_LEAGUE_URL = "https://www.tfrrs.org/leagues/76.html"
...
def main():
    # Run the Sun Belt scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for Sun Belt Conference.")
    ...
    print(f"Fetching conference page: {SUN_BELT_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, SUN_BELT_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="sun_belt_tfrrs_performances.csv",
    )
