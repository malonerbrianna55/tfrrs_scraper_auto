# TFRRS scraper for Mountain West conference track & field
...
# Mountain West conference league page
MOUNTAIN_WEST_LEAGUE_URL = "https://www.tfrrs.org/leagues/57.html"
...
def main():
    # Run the Mountain West scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for Mountain West Conference.")
    ...
    print(f"Fetching conference page: {MOUNTAIN_WEST_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, MOUNTAIN_WEST_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="mountain_west_tfrrs_performances.csv",
    )
