# TFRRS scraper for Southern conference track & field
...
# Southern conference league page
SOUTHERN_LEAGUE_URL = "https://www.tfrrs.org/leagues/80.html"
CONFERENCE_NAME = "SOUTHERN CONFERENCE"
...
def main():
    # Run the Southern Conference scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for Southern Conference.")
    ...
    print(f"Fetching conference page: {SOUTHERN_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, SOUTHERN_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="southern_c_tfrrs_performances.csv",
    )
