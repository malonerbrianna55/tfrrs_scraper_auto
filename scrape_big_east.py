# TFRRS scraper for Big East conference track & field
...
# Big 12 conference league page
BIG_EAST_LEAGUE_URL = "https://www.tfrrs.org/leagues/1463.html"
CONFERENCE_NAME = "BIG EAST"
...
def main():
    # Run the Big East scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for Big East Conference.")
    ...
    print(f"Fetching conference page: {BIG_EAST_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, BIG_EAST_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="big_east_tfrrs_performances.csv",
    )
