# TFRRS scraper for SEC conference track & field
...
SEC_LEAGUE_URL = "https://www.tfrrs.org/leagues/72.html"
CONFERENCE_NAME = "SEC"
...
def main():
    # Run the SEC scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for SEC Conference.")
    ...
    print(f"Fetching conference page: {SEC_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, SEC_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="sec_tfrrs_performances.csv",
    )
