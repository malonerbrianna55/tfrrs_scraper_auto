# TFRRS scraper for NEC conference track & field
...
NEC_LEAGUE_URL = "https://www.tfrrs.org/leagues/73.html"
CONFERENCE_NAME = "NEC"
...
def main():
    # Run the NEC scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for NEC Conference.")
    ...
    print(f"Fetching conference page: {NEC_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, NEC_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="nec_tfrrs_performances.csv",
    )
