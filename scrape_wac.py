# TFRRS scraper for WAC conference track & field
...
# WAC conference league page
WAC_LEAGUE_URL = "https://www.tfrrs.org/leagues/77.html"
CONFERENCE_NAME = "WAC"
...
def main():
    # Run the WAC scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for WAC Conference.")
    ...
    print(f"Fetching conference page: {WAC_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, WAC_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="wac_tfrrs_performances.csv",
    )
