# TFRRS scraper for Conference USA conference track & field
...
# Big West conference league page
CONFERENCE_USA_LEAGUE_URL = "https://www.tfrrs.org/leagues/67.html"
...
def main():
    # Run the Conference USA scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for Conference USA Conference.")
    ...
    print(f"Fetching conference page: {CONFERENCE_USA_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, CONFERENCE_USA_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="conference_usa_tfrrs_performances.csv",
    )
