# TFRRS scraper for Atlantic 10 conference track & field
...
# Atlantic 10 conference league page
ATLANTIC_10_LEAGUE_URL = "https://www.tfrrs.org/leagues/60.html"
CONFERENCE_NAME = "ATLANTIC 10"
...
def main():
    # Run the Atlantic 10 scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for Atlantic 10 Conference.")
    ...
    print(f"Fetching conference page: {ATLANTIC_10_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, ATLANTIC_10_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="atlantic_10_tfrrs_performances.csv",
    )
