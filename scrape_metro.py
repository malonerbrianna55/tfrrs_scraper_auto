# TFRRS scraper for Metro conference track & field
...
METRO_LEAGUE_URL = "https://www.tfrrs.org/leagues/79.html"
CONFERENCE_NAME = "METRO"
...
def main():
    # Run the Metro scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for Metro Conference.")
    ...
    print(f"Fetching conference page: {METRO_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, METRO_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="metro_tfrrs_performances.csv",
    )
