# TFRRS scraper for CAA conference track & field
...
# CAA conference league page
CAA_LEAGUE_URL = "https://www.tfrrs.org/leagues/68.html"
CONFERENCE_NAME = "CAA"
...
def main():
    # Run the CAA scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for CAA Conference.")
    ...
    print(f"Fetching conference page: {CAA_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, CAA_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="caa_tfrrs_performances.csv",
    )
