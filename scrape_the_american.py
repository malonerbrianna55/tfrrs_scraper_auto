# TFRRS scraper for America East conference track & field
...
# The American conference league page
AMERICAN_LEAGUE_URL = "https://www.tfrrs.org/leagues/1462.html"
CONFERENCE_NAME = "THE AMERICAN"
...
def main():
    # Run the American scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for The American Conference.")
    ...
    print(f"Fetching conference page: {AMERICAN_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, AMERICAN_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="american_tfrrs_performances.csv",
    )
