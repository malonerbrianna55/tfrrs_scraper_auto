# TFRRS scraper for WCC conference track & field
...
# West Coast conference league page
WCC_LEAGUE_URL = "https://www.tfrrs.org/leagues/81.html"
CONFERENCE_NAME = "WCC"
...
def main():
    # Run the WCC scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for WCC Conference.")
    ...
    print(f"Fetching conference page: {WCC_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, WCC_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="wcc_tfrrs_performances.csv",
    )
