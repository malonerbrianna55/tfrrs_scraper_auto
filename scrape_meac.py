# TFRRS scraper for MEAC conference track & field
...
MEAC_LEAGUE_URL = "https://www.tfrrs.org/leagues/71.html"
...
def main():
    # Run the MEAC scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for MEAC Conference.")
    ...
    print(f"Fetching conference page: {MEAC_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, MEAC_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="meac_tfrrs_performances.csv",
    )
