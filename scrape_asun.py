# TFRRS scraper for ASUN conference track & field
...
# ASUN conference league page
ASUN_LEAGUE_URL = "https://www.tfrrs.org/leagues/56.html"
...
def main():
    # Run the ASUN scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for ASUN Conference.")
    ...
    print(f"Fetching conference page: {ASUN_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, ASUN_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="asun_tfrrs_performances.csv",
    )
