...
BIG_12_LEAGUE_URL = "https://www.tfrrs.org/leagues/59.html"
...
def main():
    # Run the Big 12 scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for Big 12 Conference.")
    ...
    print(f"Fetching conference page: {BIG_12_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, BIG_12_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="big_12_tfrrs_performances.csv",
    )
