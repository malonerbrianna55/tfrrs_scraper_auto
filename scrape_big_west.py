# TFRRS scraper for Big West conference track & field
...
BIG_WEST_LEAGUE_URL = "https://www.tfrrs.org/leagues/66.html"
...
def main():
    # Run the Big West scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for Big West Conference.")
    ...
    print(f"Fetching conference page: {BIG_WEST_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, BIG_WEST_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="big_west_tfrrs_performances.csv",
    )
