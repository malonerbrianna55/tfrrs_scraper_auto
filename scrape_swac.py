# TFRRS scraper for SWAC conference track & field
...
SWAC_LEAGUE_URL = "https://www.tfrrs.org/leagues/75.html"
...
def main():
    # Run the SWAC scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for SWAC Conference.")
    ...
    print(f"Fetching conference page: {SWAC_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, SWAC_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="swac_tfrrs_performances.csv",
    )
