# TFRRS scraper for Patriot League conference track & field
...
# Patriot League conference league page
PAT_LEAGUE_URL = "https://www.tfrrs.org/leagues/78.html"
...
def main():
    # Run the Patriot League scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for Patriot League Conference.")
    ...
    print(f"Fetching conference page: {PAT_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, PAT_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="patriot_league_tfrrs_performances.csv",
    )
