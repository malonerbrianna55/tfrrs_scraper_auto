# TFRRS scraper for Missouri Valley conference track & field
...
# MEAC conference league page
MISSOURI_LEAGUE_URL = "https://www.tfrrs.org/leagues/83.html"
...
def main():
    # Run the Missouri Valley scrape: fetch conference page, parse team list, scrape
    # each team (rosters and performances) via scrape_tfrrs_hpu.
    # --limit for testing with fewer athletes per team
    
    parser = argparse.ArgumentParser(description="Scrape TFRRS for Missouri Valley Conference.")
    ...
    print(f"Fetching conference page: {MISSOURI_LEAGUE_URL}")
    soup = scrape_tfrrs_hpu.fetch(session, MISSOURI_LEAGUE_URL)
    if not soup:
    ...
    scrape_tfrrs_hpu.scrape_teams(
        ...
        session=session,
        limit=args.limit,
        output_path="missouri_valley_tfrrs_performances.csv",
    )
